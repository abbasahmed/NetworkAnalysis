 ...
 * all-pairs shortest paths problem in edge-weighted digraphs where the edge
 * weights are nonnegative.
 * <p>
 * This implementation runs Dijkstra's algorithm from each vertex. The
 * constructor takes time proportional to <em>V</em> (<em>E</em> log <em>V</em>)
 * and uses space proprtional to <em>V</em><sup>2</sup>, where <em>V</em> is the
 * number of vertices and <em>E</em> is the number of edges. Afterwards, the
 * {@code dist()} and {@code hasPath()} methods take constant time and the
 * {@code path()} method takes time proportional to the number of edges in the
...
public class DijkstraAllPairsSP {
	...
	public double minBW = Double.POSITIVE_INFINITY;
	 ...
	 *             unless {@code 0 <= s < V}
	 */
	public DijkstraAllPairsSP(Graph G) {
	...
	public int minBandwidth(int s, int t) {
		validateVertex(s);
		validateVertex(t);
		for (Edge e : path(s, t)) {
	...
	// public int minBandwidth(int s){
	// return all[s].minBandwidth();
	// }
